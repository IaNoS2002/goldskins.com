...
function ReportProfile( steamID )
{
	...
	var $Modal = ShowDialog( '举报个人资料', $Content );
...
var CCommentThread = Class.create( {
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		if ( !gidComment )
		{
			$modal = ShowConfirmDialog( '移除解答标记', '您确定要将该回帖取消标记为这个帖子的解答吗？您可以随时进行更新。', '移除解答标记' );
		}
		else
		{
			var strModalBody = '您即将标记此回帖为这个主题的解答。这表明原始发帖已被解答并链接至此具体回帖。如果您改变想法，可以随时移除该帖的解答标记或将另一个回帖标为解答。';
			if ( bExisting )
				strModalBody = '已有别的回帖被标记为这个主题的解答。您是否仍然要选择此回帖作为新解答？';
